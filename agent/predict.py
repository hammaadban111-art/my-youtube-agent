...
# happen silently. The repo Variable is set by hand in GitHub's UI (the
# dashboard says when the gate is waiting on it); until it reads "true" the
# mode stays off.
...
def self_improve_active(now=None) -> tuple[bool, int]:
    ...
    held by SELF_IMPROVE_AFTER) AND a one-time human approval: the
    SELF_IMPROVE_APPROVED repo Variable set to "true". Without it the mode
    stays off and the dashboard shows it as awaiting approval."""
...
def self_improve_status(now=None) -> dict:
    ...
    from "eligible and waiting on your approval", which the plain
